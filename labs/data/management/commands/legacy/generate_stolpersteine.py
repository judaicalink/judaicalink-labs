...
for j in range(0, len(explain)):
    ...
    #  print (names)
    if 'JG.' not in namecontent.split(',', 3)[2].lstrip() and 'GEB.' not in namecontent.split(',', 3)[
        2].lstrip() and 'VERH.' not in namecontent.split(',', 3)[2].lstrip():
...
for i in range(0, len(namelinklist1)):
    for j in range(0, len(picurllist)):
        if namelinklist1[i].lower() in picurllist[j].lower():
            ...
            graph.add((URIRef(uri), foaf.primaryTopic,
                       (URIRef('https://de.wikipedia.org/wiki/Liste_der_Stolpersteine_in_Mainz'))))
