...
def create_alert (submitted_search):
    #receives dictionary query_dic ["submitted_search"]
    #submitted_search may look like this: [{'Option1': 'name:', 'Input1': 'einstein'}, {'Operator3': ' OR ', 'Option3': 'birthDate:', 'Input3': '1900'}]
    #creates a string for each part of the query that will be stored in a list (alert)
    alert = []
    for dictionary in submitted_search:
        for entry in dictionary:
...
def generate_rows (submitted_search):
    counter = 0
    rows = []
    if "option" in submitted_search[0]: # to check that it is not the simple search - only the advanced search contains the option field
        while counter != len (submitted_search):
            for part in submitted_search:
                all_operators = [" AND ", " OR ", " NOT "]
                all_options = ["name:", "Alternatives:", "Publication:", "birthDate:", "deathDate:", "birthLocation:", "deathLocation:"]
                counter += 1
                row = {}
